...
function initHackathon(root, opts = {}) {
  ...
  if (fs.existsSync(hackPath)) {
    throw new Error('hackathon.json already exists — end the current hackathon first');
  }
...
function addTeam(root, teamName, question) {
  ...
  if (!hack) throw new Error('No active hackathon — run orchard hackathon init first');
  if (hack.status !== 'active') throw new Error('Hackathon is not active');

  const sprintPath = path.join('sprints', `hackathon-${teamName.toLowerCase().replace(/\s+/g, '-')}`);
  ...
  fs.writeFileSync(path.join(absPath, 'claims.json'), JSON.stringify(initialClaims, null, 2) + '\n', 'utf8');
...
function leaderboard(root) {
  ...
  const weights = hack.judging_weights || DEFAULT_HACKATHON_CONFIG.judging_weights;
  ...
  const board = hack.teams.map((team) => {
    ...
    const compilationBonus = hasCompilation ? (weights.compilation_bonus || 10) : 0;
    ...
    const score = claimScore + evidenceWeighted + compilationBonus + categoryScore;
...
function printHackathon(root) {
  ...
  if (!timer) {
    console.log('');
    console.log('  No active hackathon. Start one with: orchard hackathon init');
    console.log('');
    return;
  }
  ...
  console.log(`  Time: ${timer.elapsed}m elapsed / ${timer.remaining}m remaining (${timer.total}m total)`);
  console.log(`  Progress: ${'#'.repeat(Math.floor(timer.progress * 30))}${'·'.repeat(30 - Math.floor(timer.progress * 30))} ${Math.floor(timer.progress * 100)}%`);
  ...
  if (board.length > 0) {
    ...
    for (let i = 0; i < board.length; i++) {
      ...
      console.log(`    ${medal}  ${t.team} — ${t.score}pts (${t.claimCount} claims, evidence: ${t.evidenceScore})${compiled}`);
    }
  }
